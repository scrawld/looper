...
func TestStopBeforeStart(t *testing.T) {
	l := New()
	l.AddFunc(time.Hour, func(ctx context.Context) {
...
// TestStopWithRunningJobs tests stopping the Looper while jobs are actively running.
func TestStopWithRunningJobs(t *testing.T) {
	...
	// Stop the Looper while the job is running.
	ctx := l.Stop()
...
func TestStopDuringTask(t *testing.T) {
	...
	// Start the looper (tasks begin execution)
	l.Start()

	// Allow the task to start by waiting for 100 milliseconds
	time.Sleep(100 * time.Millisecond)

	// Stop the looper while the task is running and get the stop context
	ctx := l.Stop()
